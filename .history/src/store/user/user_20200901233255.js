...
export default {
    ...
    actions: {
        ...
        async getUsers({ commit }, { pagenum, pagesize,query }) {
            let res = await api.getUsers({ pagenum, pagesize ,query})
            //解构赋值
            commit('setUsers', res.data.users)
            commit('setTotal', res.data.total)
        },
        //添加用户
        async postAddusers({ dispatch }, { username, password, email, mobile, pagenum, pagesize,query }) {
            let res = await api.postAddusers({ username, password, email, mobile, pagenum, pagesize,query })
        ...
        //修改用户状态
        async putState({ dispatch }, { uId, type, pagenum, pagesize ,query}) {
            let res = await api.putState({ uId, type,pagenum, pagesize ,query })
            if (res.meta.status === 200) {
                Message.success(res.meta.msg)
                dispatch('getUsers', { pagenum, pagesize })
            }
        ...
        //编辑用户信息
        async putUsersid({ dispatch }, { id, email, mobile ,pagenum, pagesize,query}) {
            let res = await api.putUsersid({ id, email, mobile ,query})
            if(res.meta.status === 200){
                Message.success(res.meta.msg)
                dispatch('getUsers', { pagenum, pagesize })
            }
        ...
        //  根据id查询用户信息
        async getUsersid({ commit }, { id }) {
            // console.log(commit);
            let res = await api.getUsersid(id)
            if(res.meta.status === 200){
                 commit('setUsersid',res.data.rid)
                //  console.log(res.data.rid,'id查询用户信息');
            }
        },
        //所有角色列表
        ...
        //分配用户角色
        async putUsersRole({commit},{id,rid}){
            let res = await api.getUsersid({id,rid})
            if(res.meta.status === 200){
                commit('setUsersRole',res.data)
                console.log(res.data,'分配');
            }
        }
    },
}