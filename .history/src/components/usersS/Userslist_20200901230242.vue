<template>
  <div>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-tooltip class="item" effect="dark" content="分配角色" placement="top">
              <el-button size="mini" type="warning" @click="handleDistribution(scope.$index, scope.row)">
    ...
    <el-dialog title="分配用户" :visible.sync="dialogFormVisibleSLl" width="40%" :show-close="false">
      <div class="mb15 mt10">当前用户：{{Roleusername}}</div>
      <div>当前角色：{{Rolerole_name}}</div>
...
export default {
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      this.pagesize = val;
      //点击每一页之后再发一次请求
      this.getUsers({ pagenum: this.pagenum, pagesize: this.pagesize,query:this.query});
    },
    //分页 （当前页）
