        ...
        <el-tab-pane label="动态参数" name="first">
          ...
          >
            ...
            <template v-slot:expand="scope">
             ...
             <!--<template v-slot:expand="scope">
                <template v-if="scope.scope.attr_vals[0]!==''">
                  <el-tag
                    class="tag"
                    :key="tag"
                    v-for="tag in scope.scope.attr_vals"
                    closable
                    :disable-transitions="false"
                    @close="handleClose(tag,scope.scope.attr_vals)"
                  >{{tag}}</el-tag>
                </template>
                <el-input
                  class="input-new-tag"
                  ...
                  ref="saveTagInput"
                  size="small"
                  @keyup.enter.native="handleInputConfirm(scope.scope)"
                  @blur="handleInputConfirm(scope.scope)"
                ></el-input>
                <el-button v-else class="button-new-tag" size="small" @click="showInput(scope.scope)">+ New Tag</el-button>
              </template>-->
          </ai-table>
          <!--表单编辑-->
          <ai-from
...
export default {
  ...
  methods: {
    ...
    //   scope.splice(scope.indexOf(tag), 1);
    // },
