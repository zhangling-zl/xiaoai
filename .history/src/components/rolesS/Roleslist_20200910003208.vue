  ...
  <div>
      ...
      >
        ...
        <template v-slot:expand="scope">
          <el-tag
            ...
            @close="handleClose(item,scope.scope.row.children)"
          >
            <div class="mr20">{{item.authName}}</div>
          </el-tag>
        </template>
      </ai-table>
    </el-card>
    ...
    <el-dialog title="分配权限" :visible.sync="dialogFormVisible">
      <el-tree
        ...
        show-checkbox
        node-key="id"
        :default-expanded-keys="[101,102,103,104,105]"
        :default-checked-keys="[105,116,117,150,146,130,134,138]"
        :props="defaultProps"
      ></el-tree>
...
export default {
  ...
  mounted() {
    ...
    // console.log(this.Role.id,111);
    let idd = [];
    this.Role.map((item) => {
      // console.log(item,111,'小可爱');
      this.rights.map((item1) => {
        if (item.authName === item1.authName) {
          idd.
          console.log(this.idd,'idd');push(item1.id);
        }
      });
    });
